...
public class GoogleApi {
    ...
    public GoogleApi(String accessToken, final String spreadsheetId, String worksheetId, int startIndex,  final PropertyMapper mapper){
            ...
            public Observable<Void> call() {
                try {
                    ...
                    if (container == null ){
                        ...
                        int loadedCount = EntityManager.getInstance().getItemsCount();
                        return  containerLoading(container, loadedCount, listener);
                    }else {
                        EntityManager.getInstance().clearItemsProp();
                        container = createContainer();
                        return containerLoading(container, 0, listener);
                    }
                } catch (Exception e) {
    ...
    private ContainerProp createContainer() throws Exception {
    ...
    //    container.setDateFormat(0);
        ...
        EntityManager.getInstance().saveToDbProp(container);

        return container;
    }

    private Observable<Void> containerLoading(ContainerProp container, int alreadyLoaded, final ProgressListener listener) throws Exception {
    ...
    public Observable<Void> updateWorksheet(final List<SpreadsheetItemProp> items) {
        ...
        return Observable.defer(new Func0<Observable<Void>>() {
            ...
            public Observable<Void> call() {
                try {
                    String treadName = Thread.currentThread().getName() + " " + Thread.currentThread().getId();
                    Log.e("prepare", treadName);
                    ItemsConverter converter = new ItemsConverter(Integer.valueOf(worksheetId), mapper);
